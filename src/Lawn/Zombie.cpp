...
ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {  
    ...
    { ZOMBIE_BRICK,             REANIM_BRICK,                4,     51,     1,      3000,   _S("BUG")},
    { ZOMBIE_BUG,               REANIM_17,                   2,     53,     1,      1000,   _S("COCOON")},
    { ZOMBIE_GIGA_FOOTBALL,     REANIM_7,                   7,      56,     5,      2000,   _S("GIGA_FOOTBALL_ZOMBIE")},
    { ZOMBIE_BLEUEYE_GARGANTUAR,REANIM_GARGANTUAR,          10,     58,     15,     6000,   _S("BLEUEYED_GARGANTUAR")}, 
    { ZOMBIE_BOSS,              REANIM_BOSS,                10,     60,     1,      0,      _S("BOSS")},
    { ZOMBIE_REDEYE_GARGANTUAR, REANIM_GARGANTUAR,          10,     99,     15,     6000,   _S("REDEYED_GARGANTUAR")},
    { ZOMBIE_NINJA,             REANIM_BOBSLED,            10,     99,     1,      1500,  _S("NINJA")},
    { ZOMBIE_EXA_GARGANTUAR,    REANIM_GARGANTUAR,          10,     99,     15,     6000,   _S("EXA_GARGANTUAR")},
    ...
    { ZOMBIE_FLY,               REANIM_18,                   2,     99,     1,      1000,   _S("FLY")},
    { ZOMBIE_COCOON,            REANIM_19,                   2,     99,     1,      1000,   _S("COCOON")},
    { ZOMBIE_SUNDYEDITION,      REANIM_ZOMBIE_NEWSPAPER,     2,     99,     1,      1000,   _S("SUNDAYEDITION") }
};
...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave, bool theAnimateBush)
{
    ...
    mCanDropNuclearGrave = false;
    mHeight = 120;
    mFrame = 0;
    mPrevFrame = 0;
    mZombieType = theType;
    mIsNovaBurning = false;
    mNovaBurnTimer = 0;
    mVariant = theVariant;
    mIsEating = false;
    ...
    mZombieFade = -1;
    mFlatTires = false;
    mIsNuclearBall = false;
    mIsVacuumBall = false;
    mNuclearDelayCounter = 0;
    mHasPlantFood = false; 
    mScaleZombie = 1.0f;
    mUseLadderCol = -1;
    mShieldHealth = 0;
    ...
    case ZombieType::ZOMBIE_PAIL:  
        ...
        mHelmHealth = 1100;
        break;
    case ZombieType::ZOMBIE_BRICK:
        LoadPlainZombieReanim();
        ReanimShowPrefix("anim_brick", RENDER_GROUP_NORMAL);
        ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
        mHelmType = HelmType::HELMTYPE_BRICK;
        mHelmHealth = 2200;
        break;
  case ZombieType::ZOMBIE_BUG:
        mBodyHealth = 40;
        mBodyMaxHealth = 40;
        mScaleZombie = (theFromWave == Zombie::ZOMBIE_WAVE_UI) ? 1.0f : 0.25f; 
        mZombieRect = Rect(0, 0, 40, 40);
        mZombieAttackRect = Rect(0, 0, 40, 40);
        mPlantsEaten = 0;
        break;
    case ZombieType::ZOMBIE_FLY:
        mBodyHealth = 400;
        mBodyMaxHealth = 400;
        mZombieRect = Rect(0, 0, 50, 50);
        mZombieAttackRect = Rect(0, 0, 50, 50);
        mTargetPlantID = PlantID::PLANTID_NULL;
        break;
    case ZombieType::ZOMBIE_COCOON:
        mBodyHealth = 10;
        mBodyMaxHealth = 100;
        mVelX = 0.1f;
        mPhaseCounter = 1500;
    ...
    case ZombieType::ZOMBIE_GIGA_FOOTBALL:
    {
        ...
        if (aBodyReanim)
        {
            aBodyReanim->SetImageOverride("zombie_football_helmet", IMAGE_REANIM_ZOMBIE_GIGA_FOOTBALL_HELMET1);
        }
        break;
    }
    case ZombieType::ZOMBIE_NINJA:
    {
        mBodyHealth = 1350;
        mBodyMaxHealth = 1350;
        mVelX = 1.0f;
        ...
        if (aBodyReanim)
        {
            aBodyReanim->SetImageOverride("anim_head1", IMAGE_REANIM_NINJA_HEAD);
        }
        break;
    }
    ...
    case ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR:
    case ZombieType::ZOMBIE_EXA_GARGANTUAR:
    {
        mWidth = 180;
        mHeight = 180;
        mBodyHealth = 3000;
        ...
        if (mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
        {
            aBodyReanim->SetImageOverride("anim_head1", IMAGE_TERA1);
            mBodyHealth = 12000;
            mScaleZombie = 1.1f;
        }
        if (mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
        {
            aBodyReanim->SetImageOverride("anim_head1", IMAGE_EXA1);
            mBodyHealth = 24000;
            mScaleZombie = 1.2f;
        }
        break;
    ...
    case ZombieType::ZOMBIE_NEWSPAPER:  
        ...
        AttachShield();
        break;
    case ZombieType::ZOMBIE_SUNDYEDITION:
    {
        mZombieAttackRect = Rect(20, 0, 50, 115);
        mZombiePhase = ZombiePhase::PHASE_NEWSPAPER_READING;
        mShieldType = ShieldType::SHIELDTYPE_SUNDYNEWSPAPER;
        mBodyHealth = 1100;
        mShieldHealth = 2200;
        mVariant = false;
        AttachShield();
        ...
        if (aBodyReanim)
        {
            aBodyReanim->SetImageOverride("Zombie_paper_paper", IMAGE_NOBLE_PAPER_PAPER1);
            aBodyReanim->SetImageOverride("Zombie_paper_body", IMAGE_NOBLE_PAPER_BODY);
            aBodyReanim->SetImageOverride("Zombie_paper_leftarm_upper", IMAGE_NOBLE_PAPER_LEFTARM_UPPER);
            aBodyReanim->SetImageOverride("Zombie_paper_leftarm_lower", IMAGE_NOBLE_PAPER_LEFTARM_LOWER);
            aBodyReanim->SetImageOverride("Zombie_outerarm_upper", IMAGE_NOBLE_PAPER_RIGHTARM_UPPER);
            aBodyReanim->SetImageOverride("Zombie_outerarm_lower", IMAGE_NOBLE_PAPER_RIGHTARM_LOWER);
        }
        break;
    }
    ...
    case ZombieType::ZOMBIE_BOSS:  
        ...
        mBodyHealth = mApp->IsAdventureMode() ? 200000 : 400000;
        if (IsOnBoard())
        {
...
void Zombie::SetupReanimLayers(Reanimation* aReanim, ZombieType theZombieType)
{
    ...
    else if (theZombieType == ZombieType::ZOMBIE_BRICK)
    {
        aReanim->AssignRenderGroupToPrefix("anim_brick", RENDER_GROUP_NORMAL);
        aReanim->AssignRenderGroupToPrefix("anim_hair", RENDER_GROUP_HIDDEN);
    }
    ...
    else if (theZombieType == ZombieType::ZOMBIE_SUNDYEDITION)
    {
        aReanim->AssignRenderGroupToPrefix("Zombie_paper_paper", RENDER_GROUP_HIDDEN);
    }
...
void Zombie::PickRandomSpeed()
{
    if (mZombiePhase == ZombiePhase::PHASE_DOLPHIN_WALKING_IN_POOL) {
        ...
        mVelX = mApp->IsIZombieLevel() ? 0.23f : 0.12f;
    }
    ...
    else if (mZombieType == ZombieType::ZOMBIE_FLY) {
        mVelX = 2.0f;
    }
    else if (mZombieType == ZombieType::ZOMBIE_BUG) {
        mVelX = 0.12f;
    }
        ...
        if (mZombieType == ZombieType::ZOMBIE_SUNDYEDITION) {
            mVelX = RandRangeFloat(2.5f, 2.6f);
        }
        else {
            mVelX = RandRangeFloat(1.2f, 1.3f);
        }
    }
    else if (mZombiePhase == ZombiePhase::PHASE_NEWSPAPER_READING) {
        if (mZombieType == ZombieType::ZOMBIE_SUNDYEDITION) {
            mVelX = RandRangeFloat(0.35f, 0.45f);
        }
        else {
            mVelX = RandRangeFloat(0.35f, 0.45f);
        }
    ...
    else if (mZombieType == ZombieType::ZOMBIE_NINJA) {
        mVelX = 1.0f;
    }
...
void Zombie::UpdateZombiePogo()
{
    ...
    if (mZombiePhase == ZombiePhase::PHASE_POGO_FORWARD_BOUNCE_2 && mPhaseCounter == 70)
    {
        ...
        if (aPlant)
        {
            ...
            else if (aPlant->mSeedType == SeedType::SEED_FLOWERPOT)
            {
                int potCount = 0;
                Plant* p = nullptr;
                while (mBoard->IteratePlants(p))
                {
                    if (p->mPlantCol == aPlant->mPlantCol && p->mRow == aPlant->mRow && p->mSeedType == SeedType::SEED_FLOWERPOT && !p->mDead)
                    {
                        potCount++;
                    }
                }
                if (potCount > 2) isTall = true;
            }
...
void Zombie::UpdateZombieSnorkel()
{
    if (mInPool && !IsDeadOrDying())
    {
        ...
        while (mBoard->IteratePlants(aPlant))
        {
            if (aPlant->mRow == mRow && aPlant->mSeedType == SeedType::SEED_TANGLEKELP)
            {
                int dist = aGridX - aPlant->mPlantCol;
                if (dist >= 0 && dist <= 2) 
                {
                    dangerAhead = true;
                    break;
        ...
        if (dangerAhead)
        {
            int newRow = (mRow == 2) ? 3 : (mRow == 3 ? 2 : -1);
            if (newRow != -1)
            {
                bool isSafe = true;
                ...
                {
                    if (aCheckPlant->mRow == newRow && aCheckPlant->mSeedType == SeedType::SEED_TANGLEKELP)
                    {
                        int dist = aGridX - aCheckPlant->mPlantCol;
                        if (dist >= 0 && dist <= 2)
                        {
                            isSafe = false;
                            break;
...
void Zombie::UpdateZombieGargantuar()
{
            ...
            else
            {
                ...
                if (aPlant)
                {
                    if (mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
                    {
                        ...
                        {
                            if (!z->IsDeadOrDying() && !z->mMindControlled)
                            {
                                z->mVelX *= 1.5f;
                                z->mAnimTicksPerFrame = max(1, (int)(z->mAnimTicksPerFrame / 1.5f));
                                z->UpdateAnimSpeed();
                            }
                        }
                        int currentGridX = mBoard->PixelToGridXKeepOnBoard(mX, mY);
                        GridItem* crater = mBoard->AddACrater(currentGridX - 1, mRow);
                        if (crater) crater->mGridItemCounter = 4500;

                        mApp->AddTodParticle(BOARD_WIDTH / 2, BOARD_HEIGHT / 2, (int)RenderLayer::RENDER_LAYER_TOP, ParticleEffect::PARTICLE_SCREEN_FLASH);
                ...
                Zombie* aTargetZombie = FindZombieTarget();
                if (aTargetZombie)
                {
                    int aDamage = 600;
                    aTargetZombie->TakeDamage(aDamage, 0U);
                }
                if (mApp->IsScaryPotterLevel())
                {
    ...
    if (mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
    {
        ...
        while (mBoard->IterateZombies(z))
        {
            if (z != this && z->mRow == mRow && !z->IsDeadOrDying())
            {
                float dist = mPosX - z->mPosX;
                if (dist >= 80.0f && dist <= 120.0f)
                {
                    targetZombie = z;
                    break;
                }
        ...
        if (targetZombie)
        {
            ...
            mTargetZombieID = mBoard->ZombieGetID(targetZombie);
...
void Zombie::UpdateZombieDigger()
{
    ...
    else if (mZombiePhase == ZombiePhase::PHASE_DIGGER_RISING)
    {
        ...
        if (mPhaseCounter == 0)
        {
            ...
            GridItem* aCrater = mBoard->AddACrater(mBoard->PixelToGridXKeepOnBoard(mX, mY), mRow);
            if (aCrater) aCrater->mGridItemCounter = 9000;
            int craterGridX = mBoard->PixelToGridXKeepOnBoard(mX, mY);
            Plant* aPlantToKill = mBoard->GetTopPlantAt(craterGridX, mRow, PlantPriority::TOPPLANT_ANY);
            if (aPlantToKill)
            {
                aPlantToKill->Die();
            }
...
bool Zombie::CanLoseBodyParts()
{
    return 
        ...
        mZombieType != ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR &&
        mZombieType != ZombieType::ZOMBIE_EXA_GARGANTUAR &&
...
void Zombie::DropHead(unsigned int theDamageFlags)
{
    ...
    else if (mZombieType == ZombieType::ZOMBIE_SUNDYEDITION)
    {
        aEffect = ParticleEffect::PARTICLE_ZOMBIE_NEWSPAPER_HEAD;
    }
...
void Zombie::DropArm(unsigned int theDamageFlags)
{
    ...
    if (mShieldType == ShieldType::SHIELDTYPE_DOOR || mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER || mShieldType == ShieldType::SHIELDTYPE_NEWSPAPER)
    {
        return;
    }
...
void Zombie::UpdateZombieWalking()
{
    ...
    if (aBodyReanim)
    {
        ...
        if (mBoard && mBoard->mEclipseCounter > 0) aSpeed *= 1.25f;
    ...
    else
    {
        ...
        if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_IN_VAULT ||
            ...
            (mZombieType == ZombieType::ZOMBIE_SNORKEL && mInPool) ||
            (mFrame >= 0 && mFrame <= 2) ||
            (mFrame >= 6 && mFrame <= 8))
        {
            doWalk = true;
        ...
        {
            float aSpeed = mVelX;
            if (IsMovingAtChilledSpeed()) aSpeed *= CHILLED_SPEED_FACTOR;
            if (mBoard && mBoard->mEclipseCounter > 0) aSpeed *= 1.25f;
...
void Zombie::CheckForZombieStep()
{
    if ((mZombieType == ZombieType::ZOMBIE_ZAMBONI || mZombieType == ZombieType::ZOMBIE_CATAPULT) && !mFlatTires)
    {
        ...
        {
            Zombie* otherZ = nullptr;
            while (mBoard->IterateZombies(otherZ))
            {
                if (otherZ != this && !otherZ->IsDeadOrDying() && otherZ->mRow == mRow && otherZ->mZombieType == ZombieType::ZOMBIE_ZAMBONI)
                {
                    if (mMindControlled != otherZ->mMindControlled)
                    {
                        Rect rect1 = GetZombieRect();
                        Rect rect2 = otherZ->GetZombieRect();
                        if (GetRectOverlap(rect1, rect2) > 0)
                        {
                            this->TakeDamage(3000, 0U);
                            otherZ->TakeDamage(3000, 0U);
                            break;
                        }
                    }
                }
...
void Zombie::Update()
{
    ...
    if (doUpdate)
    {
        if (mIsNovaBurning && !IsDeadOrDying())
        {
            mNovaBurnTimer--;
            if (mNovaBurnTimer <= 0)
            {
        ...
        else if (mZombieType == ZombieType::ZOMBIE_COCOON && !IsDeadOrDying())
        {
            if (mBoard != nullptr && mBoard->mEclipseCounter > 0) 
            {
                if (mPhaseCounter > 0) mPhaseCounter--;
                if (mPhaseCounter <= 0)
                {
                    Zombie* z = mBoard->AddZombieInRow(ZombieType::ZOMBIE_BUG, mRow, mFromWave, true);
                    if (z) { z->mPosX = mPosX; z->mPosY = mPosY; }
                    DieNoLoot();
                }
            }
        }
...
void Zombie::UpdateActions()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_NINJA)
    {
        UpdateZombieNinja();
    }
    ...
    if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUNDYEDITION)
    {
    ...
    if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
    {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_TRAMPALINE_BOUNCE)
    {
        mVelZ -= 0.05f;
        mAltitude += mVelZ;
        mPosX += mVelX;
        ...
        {
            mAltitude = 0.0f;
            mZombiePhase = (ZombiePhase)mLastPortalX; 
            PickRandomSpeed(); 
            TakeDamage(150, 0U);
        }
    }
...
void Zombie::UpdatePlaying()
{
    ...
    if (mApp->IsFootballLevel() && mZombieType == ZombieType::ZOMBIE_FOOTBALL)
    {
        Projectile* aBall = nullptr;
        Projectile* aProj = nullptr;
        while (mBoard->IterateProjectiles(aProj))
        {
            if (aProj->mProjectileType == ProjectileType::PROJECTILE_BASKETBALL)
            {
                aBall = aProj;
                break;
            }
        ...
        if (aBall)
        {
            float targetY = aBall->mPosY;
            float zombieCenterY = mPosY + 60.0f;
            if (aBall->mVelX > 0)
            {
                float timeToReach = (mPosX - aBall->mPosX) / aBall->mVelX;
                if (timeToReach > 0)
                {
                    targetY = aBall->mPosY + aBall->mVelY * timeToReach;
                    while (targetY < 80.0f || targetY > 450.0f)
                    {
                        if (targetY < 80.0f) targetY = 80.0f + (80.0f - targetY);
                        if (targetY > 450.0f) targetY = 450.0f - (targetY - 450.0f);
                    }
                }
            }
            float speed = RandRangeFloat(1.0f, 2.0f);
            if (zombieCenterY < targetY - 45.0f)
            {
                mPosY += speed;
            }
            else if (zombieCenterY > targetY + 45.0f)
            {
                mPosY -= speed;
            }
        ...
        mPosX = mBoard->GridToPixelX(6, mRow) + 20.0f;
        mX = (int)mPosX;
        Animate();
        return;
    }
...
bool Zombie::HasYuckyFaceImage()
{
    ...
    return 
        ...
        mZombieType == ZombieType::ZOMBIE_SUNDYEDITION||
        mZombieType == ZombieType::ZOMBIE_POLEVAULTER;
}
...
void Zombie::AnimateChewSound()
{
    ...
    if (aPlant)
    {
        ...
        else if (aPlant->mSeedType == SeedType::SEED_SUN_BAN)
        {
            mApp->PlayFoley(FoleyType::FOLEY_CHOMP);
            aPlant->Die();
            mSunBanMarked = true;
...
void Zombie::Animate()
{
    ...
    if (mIsEating && mHasHead)
    {
        ...
        if (aBodyReanim)
        {
            ...
            else if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUNDYEDITION || mZombieType == ZombieType::ZOMBIE_LADDER)
            {
                aLeftHandTime = 0.42f;
...
bool Zombie::IsWalkingBackwards()
{
    if (mMindControlled)
    {
        if (mZombieType == ZombieType::ZOMBIE_COCOON) return false;
        return true;
    }
    if (mZombieHeight == ZombieHeight::HEIGHT_ZOMBIQUARIUM)
    {
    ...
    if (mZombieType == ZombieType::ZOMBIE_YETI && !mHasObject)
        return true;
    if (mZombieType == ZombieType::ZOMBIE_COCOON)
        return true;
    return false;
}
...
void Zombie::UpdateReanim()
{
    ...
    if (mBoard && mBoard->mEclipseCounter > 0 && mHasHead && !Zombie::IsZombotany(mZombieType))
    {
        int aThreshold = 1500 - ((mBoard->ZombieGetID(this) % 50) * 10);
        if (mBoard->mEclipseCounter <= aThreshold)
        {
            aBodyReanim->SetImageOverride("anim_head1", IMAGE_REANIM_ZOMBIE_HEAD_SUNGLASSES1);
        }
    }
    bool aCanWearGlasses = mHasHead && !Zombie::IsZombotany(mZombieType) &&
        mZombieType != ZombieType::ZOMBIE_CATAPULT && mZombieType != ZombieType::ZOMBIE_BALLOON &&
        mZombieType != ZombieType::ZOMBIE_IMP && mZombieType != ZombieType::ZOMBIE_BUG &&
        mZombieType != ZombieType::ZOMBIE_COCOON && mZombieType != ZombieType::ZOMBIE_GARGANTUAR &&
        mZombieType != ZombieType::ZOMBIE_REDEYE_GARGANTUAR && mZombieType != ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR &&
        mZombieType != ZombieType::ZOMBIE_EXA_GARGANTUAR && mZombieType != ZombieType::ZOMBIE_BOBSLED &&
        mZombieType != ZombieType::ZOMBIE_DIGGER && mZombieType != ZombieType::ZOMBIE_BUNGEE;

    if (mBoard && mBoard->mEclipseCounter > 0 && aCanWearGlasses)
    {
        int aDelay = (mBoard->ZombieGetID(this) % 50) * 10;
        if (mBoard->mEclipseCounter <= 20000 - aDelay && mBoard->mEclipseCounter >= aDelay)
        {
            aBodyReanim->SetImageOverride("anim_head1", IMAGE_REANIM_ZOMBIE_HEAD_SUNGLASSES1);
        }
        else
    ...
    else if (mBoard && !mBoard->mFutureMode && mHasHead && !Zombie::IsZombotany(mZombieType))
    {
        if (aBodyReanim->GetImageOverride("anim_head1") == IMAGE_REANIM_ZOMBIE_HEAD_SUNGLASSES1)
        {
            aBodyReanim->SetImageOverride("anim_head1", nullptr);
            ...
            if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
            {
                if (aDamageIndexAfterDamage == 2)
                {
                    if (mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR) aBodyReanim->SetImageOverride("anim_head1", IMAGE_TERA2);
                    else if (mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR) aBodyReanim->SetImageOverride("anim_head1", IMAGE_EXA2);
                    else if (mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR) aBodyReanim->SetImageOverride("anim_head1", IMAGE_REANIM_ZOMBIE_GARGANTUAR_HEAD2_REDEYE);
                    else aBodyReanim->SetImageOverride("anim_head1", IMAGE_REANIM_ZOMBIE_GARGANTUAR_HEAD2);
                }
            }
            else if (mZombieType == ZombieType::ZOMBIE_BOSS)
            {
                if (aDamageIndexAfterDamage == 1) aBodyReanim->SetImageOverride("Boss_head", IMAGE_REANIM_ZOMBIE_BOSS_HEAD_DAMAGE1);
                else if (aDamageIndexAfterDamage == 2) aBodyReanim->SetImageOverride("Boss_head", IMAGE_REANIM_ZOMBIE_BOSS_HEAD_DAMAGE2);
            }
        }
    }
    aBodyReanim->Update();
    ...
    if (mZombieType == ZombieType::ZOMBIE_BOSS)
    {
        ReanimatorTrackInstance* eyeTrack = aBodyReanim->GetTrackInstanceByName("Boss_eyeglow_red");
        ReanimatorTrackInstance* mouthTrack = aBodyReanim->GetTrackInstanceByName("Boss_mouthglow_red");
        if (mIsNuclearBall)
        {
            if (eyeTrack) eyeTrack->mTrackColor = Color(0, 255, 0, 255);
            if (mouthTrack) mouthTrack->mTrackColor = Color(0, 255, 0, 255);
        ...
        else if (mIsVacuumBall)
        {
            if (eyeTrack) eyeTrack->mTrackColor = Color(160, 32, 240, 255); 
            if (mouthTrack) mouthTrack->mTrackColor = Color(160, 32, 240, 255); 
            aBodyReanim->SetImageOverride("Boss_eyeglow_red", IMAGE_REANIM_ZOMBIE_BOSS_EYEGLOW_BLUE);
            aBodyReanim->SetImageOverride("Boss_mouthglow_red", IMAGE_REANIM_ZOMBIE_BOSS_MOUTHGLOW_BLUE);
        }
        else
        {
            if (eyeTrack) eyeTrack->mTrackColor = Color::White;
            if (mouthTrack) mouthTrack->mTrackColor = Color::White;
        }
    }
}
...
void Zombie::DrawReanim(Graphics* g, const ZombieDrawPosition& theDrawPos, int theBaseRenderGroup)
{
    ...
    if (mIsRadioactive)
    {
        aColorOverride = Color(90, 230, 90);
    }
    else if (mIsNovaBurning)
    {
        aColorOverride = Color(255, 64, 0, aFadeAlpha); 
        aExtraAdditiveColor = Color(150, 40, 0);
        aEnableExtraAdditiveDraw = true;
    }
    else if (mIsSpyPepper)
    {
        aColorOverride = Color(200, 100, 100, aFadeAlpha);
        aExtraAdditiveColor = Color(50, 0, 0);
        aEnableExtraAdditiveDraw = true;
    }
    else  if (mZombieType == ZombieType::ZOMBIE_NINJA)
    {
        aColorOverride = Color(255, 255, 255, aFadeAlpha);
        aExtraAdditiveColor = Color(0, 0, 0, 0);
        aEnableExtraAdditiveDraw = false;
        Reanimation* aBodyReanimLoc = mApp->ReanimationGet(mBodyReanimID);
        if (aBodyReanimLoc)
        {
            for (int i = 0; i < aBodyReanimLoc->mDefinition->mTrackCount; i++)
            {
                ReanimatorTrackInstance* aTrackInst = aBodyReanimLoc->GetTrackInstanceByName(aBodyReanimLoc->mDefinition->mTracks[i].mName);
                const char* aTrackName = aBodyReanimLoc->mDefinition->mTracks[i].mName;

                if (aTrackInst)
                {
                    if (strstr(aTrackName, "head") == nullptr && strstr(aTrackName, "jaw") == nullptr)
                    {
                        aTrackInst->mTrackColor = Color(0, 0, 0, 255);
                    }
                    else
                    {
                        aTrackInst->mTrackColor = Color(255, 255, 255, 255);
                    }
                }
            }
        }
    }
    else if (mHasPlantFood)
    {
        int aPulse = TodAnimateCurve(0, 50, mBoard->mMainCounter % 50, 150, 255, TodCurves::CURVE_BOUNCE);
        aColorOverride = Color(aPulse / 2, aPulse, aPulse / 2, aFadeAlpha);
        aExtraAdditiveColor = Color(aPulse / 4, aPulse / 2, aPulse / 4);
        aEnableExtraAdditiveDraw = true;
    }
    else if (mSunBanMarked)
    {
        int aPulse = TodAnimateCurve(0, 50, mBoard->mMainCounter % 50, 150, 255, TodCurves::CURVE_BOUNCE);
        aColorOverride = Color(aPulse, aPulse, 0, aFadeAlpha);
        aExtraAdditiveColor = Color(aPulse / 2, aPulse / 2, 0);
        aEnableExtraAdditiveDraw = true;
    }
    else if (mAcidCounter > 0)
    {
        int aPulse = TodAnimateCurve(0, 50, mBoard->mMainCounter % 50, 150, 255, TodCurves::CURVE_BOUNCE);
        aColorOverride = Color(160, 32, 240, aFadeAlpha);
        aExtraAdditiveColor = Color(aPulse / 3, 0, aPulse / 2);
        aEnableExtraAdditiveDraw = true;
    }
    ...
    else if (mJustGotShotCounter > 0 && !IsBobsledTeamWithSled() && mZombieType != ZombieType::ZOMBIE_BOSS)
    {
        int aGrayness = mJustGotShotCounter * 10;
    ...
    {
        aColorOverride = Color(100, 160, 220, aFadeAlpha);
        aExtraAdditiveColor = Color(30, 60, 110);
        aEnableExtraAdditiveDraw = true;
    }
    ...
    if (mShieldType == ShieldType::SHIELDTYPE_NEWSPAPER || mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER || mShieldType == ShieldType::SHIELDTYPE_DOOR || mShieldType == ShieldType::SHIELDTYPE_LADDER)
    {
        aBodyReanim->mColorOverride = aColorOverride;
        aBodyReanim->mExtraAdditiveColor = aExtraAdditiveColor;
        aBodyReanim->mEnableExtraAdditiveDraw = aEnableExtraAdditiveDraw;
...
bool Zombie::CanTargetPlant(Plant* thePlant, ZombieAttackType theAttackType)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_FLY)
    {
        if (thePlant->mSeedType == SeedType::SEED_TALLNUT ||
            thePlant->mSeedType == SeedType::SEED_GUARDIAN_SHROOM ||
            thePlant->mSeedType == SeedType::SEED_MINI_GUARDIAN_SHROOM)
        {
            return true;
        }
        return false;
    ...
    if (mZombiePhase == ZombiePhase::PHASE_LADDER_CARRYING || mZombiePhase == ZombiePhase::PHASE_LADDER_PLACING)
    {
        ...
        if (!thePlant->IsSpiky() && !thePlant->NotOnGround() && thePlant->mSeedType != SeedType::SEED_COBCANNON)
        {
            aPlaceLadder = true;
        }
    ...
    if (mZombieType == ZombieType::ZOMBIE_SNORKEL && mInPool && !mIsEating)
    {
        if (thePlant->mSeedType == SeedType::SEED_WALLNUT ||
            ...
            thePlant->mSeedType == SeedType::SEED_GIANT_WALLNUT)
        {
            return false; 
        }
    }
...
void Zombie::SquishAllInSquare(int theX, int theY, ZombieAttackType theAttackType)
{
    ...
    while (mBoard->IteratePlants(aPlant))
    {
        ...
        {
            if (aPlant->mHasBooster) continue;
            if (theAttackType == ZombieAttackType::ATTACKTYPE_DRIVE_OVER && aPlant->IsSpiky()) continue;

            if (theAttackType != ZombieAttackType::ATTACKTYPE_DRIVE_OVER && aPlant->mSeedType == SeedType::SEED_TALLNUT)
            {
                aPlant->mPlantHealth -= 600;
                if (aPlant->mPlantHealth <= 0)
                {
                    mBoard->mPlantsEaten++;
                    aPlant->Squish();
...
void Zombie::ApplyAnimRate(float theAnimRate)
{
    ...
    if (aBodyReanim)
    {
        float aRate = IsMovingAtChilledSpeed() ? theAnimRate * 0.5f : theAnimRate;
...
void Zombie::StartWalkAnim(int theBlendTime)
{
    ...
    else if ((mZombieType == ZombieType::ZOMBIE_NORMAL || mZombieType == ZombieType::ZOMBIE_TRAFFIC_CONE || mZombieType == ZombieType::ZOMBIE_BRICK || mZombieType == ZombieType::ZOMBIE_PAIL) && mBoard->mDanceMode)
    {
    ...
    if (mMindControlled)
    {
        ...
        while (mBoard->IterateGridItems(aGridItem))
        {
            if (aGridItem->mGridItemType == GridItemType::GRIDITEM_PVZ2_GRAVE && aGridItem->mGridY == mRow)
            {
                ...
                Rect aZombieAttackRect = GetZombieAttackRect();
                if (GetRectOverlap(aZombieAttackRect, aGraveRect) > 0)
                {
                    aGridItem->TakeDamage(DAMAGE_PER_EAT);
                    StartEating();
                    mApp->PlayFoley(FoleyType::FOLEY_CHOMP);
                    return;
                }
...
{
    if (thePlant->mHasBooster && thePlant->mSeedType != SeedType::SEED_HYPNOSHROOM) return;
    ...
    StartEating();
    if (thePlant->mSeedType == SeedType::SEED_GUARDIAN_SHROOM)
    {
        TakeDamage(1, 0U);
    }
    else if (thePlant->mSeedType == SeedType::SEED_SUN_BAN)
    {
        if (thePlant->mHasBooster)
        {
            TodParticleSystem* p = mApp->AddTodParticle(mX + 40.0f, mY + 40.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_POWIE);
            if (p)
            {
                p->OverrideScale(nullptr, 0.5f);
            }
            ApplyBurn();
            DieWithLoot();
            thePlant->Die();
    ...
    int aDamageToPlant = DAMAGE_PER_EAT;
    if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER)
    {
        aDamageToPlant = DAMAGE_PER_EAT * 4; 
    }
    else if (mZombieType == ZombieType::ZOMBIE_SUNDYEDITION)
    {
        aDamageToPlant = (int)(DAMAGE_PER_EAT * 5); 
    }
    else if (mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL)
    {
        aDamageToPlant = DAMAGE_PER_EAT * 2; 
    }
    thePlant->mPlantHealth -= aDamageToPlant;
    ...
    if (mZombieType == ZombieType::ZOMBIE_BUG)
    {
        int aDamageToPlantBug = aDamageToPlant / 4;
        if (aDamageToPlantBug < 1) aDamageToPlantBug = 1;
        thePlant->mPlantHealth -= aDamageToPlantBug;
        if (thePlant->mPlantHealth <= 0)
        {
            if (thePlant->mSeedType == SeedType::SEED_POISONSHROOM)
            {
                ApplyAcid();
            }
            mApp->PlaySample(SOUND_GULP);
            if (mBoard && mBoard->mNuclearRainActive)
            {
                if (mBoard->GetGridItemAt(GridItemType::GRIDITEM_INFECTION, thePlant->mPlantCol, thePlant->mRow) == nullptr)
                {
                    GridItem* aInfection = mBoard->AddInfectionGridItem(thePlant->mPlantCol, thePlant->mRow);
                    if (aInfection)
                    {
                        aInfection->mGridItemCounter = 1500;
                        aInfection->mPosX = thePlant->mX;
                        aInfection->mPosY = thePlant->mY;
                    }
                }
            }
            mPlantsEaten++;
            ...
            {
                mScaleZombie = 0.5f;
                mZombieRect = Rect(0, 0, 60, 60);
                mZombieAttackRect = Rect(0, 0, 60, 60);
                mBodyHealth = 400;
                mBodyMaxHealth = 400;
                mVelX *= 1.5f;
                mAnimTicksPerFrame = 6;
            ...
            else if (mPlantsEaten == 3)
            {
                mScaleZombie = 1.0f;
                mZombieRect = Rect(0, 0, 120, 120);
                mZombieAttackRect = Rect(0, 0, 120, 120);
                mBodyHealth = 800;
                mBodyMaxHealth = 800;
                mVelX *= 0.8f;
                mAnimTicksPerFrame = 8;
        ...
        if (mBoard && mBoard->mNuclearRainActive)
        {
            if (mBoard->GetGridItemAt(GridItemType::GRIDITEM_INFECTION, thePlant->mPlantCol, thePlant->mRow) == nullptr)
            {
                ...
                if (aInfection)
                {
                    ...
                    aInfection->mPosX = thePlant->mX;
                    aInfection->mPosY = thePlant->mY;
                    TodParticleSystem* aEffect = mApp->AddTodParticle(aInfection->mPosX + 40, aInfection->mPosY + 40, mBoard->MakeRenderOrder(RenderLayer::RENDER_LAYER_PLANT, thePlant->mRow, 0), ParticleEffect::PARTICLE_IMITATER_MORPH);
...
void Zombie::DropLoot()
{
    ...
    {
        mHasPlantFood = false;
        mBoard->AddCoin(mPosX + mWidth / 2, mPosY + mHeight / 2, CoinType::COIN_PLANT_FOOD, CoinMotion::COIN_MOTION_COIN);
    }
    Rect aZombieRect = GetZombieRect();
    int aCenterX = aZombieRect.mX + aZombieRect.mWidth / 2;
...
void Zombie::DropShield(unsigned int theDamageFlags)
{
    ...
    else if (mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER)
    {
        StopEating();
        if (mYuckyFace)
        {
...
int Zombie::TakeShieldDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if (aDamageIndexAfterDamage != aDamageIndexBeforeDamage)
    {
        ...
        else if (mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER && aDamageIndexAfterDamage == 1)
        {
            TOD_ASSERT(aBodyReanim);
            aBodyReanim->SetImageOverride("Zombie_paper_paper", IMAGE_NOBLE_PAPER_PAPER2);
        }
        else if (mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER && aDamageIndexAfterDamage == 2)
        {
            TOD_ASSERT(aBodyReanim);
            aBodyReanim->SetImageOverride("Zombie_paper_paper", IMAGE_NOBLE_PAPER_PAPER2);
        }
...
void Zombie::DropHelm(unsigned int theDamageFlags)
{
    ...
    {
        GetTrackPosition("anim_brick", aPosX, aPosY);
        ReanimShowPrefix("anim_brick", RENDER_GROUP_HIDDEN);
        ReanimShowPrefix("anim_hair", RENDER_GROUP_NORMAL);
        aEffect = ParticleEffect::PARTICLE_ZOMBIE_PAIL;
    }
    ...
    if (!TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_DOESNT_LEAVE_BODY) && aEffect != ParticleEffect::PARTICLE_NONE)
    {
        TodParticleSystem* aParticle = mApp->AddTodParticle(aPosX, aPosY, mRenderOrder + 1, aEffect);
        OverrideParticleScale(aParticle);
        if (mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL && aParticle)
        {
            aParticle->OverrideImage(nullptr, IMAGE_REANIM_ZOMBIE_GIGA_FOOTBALL_HELMET1);
        }
    }
...
int Zombie::TakeHelmDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if (aDamageIndexBeforeDamage != aDamageIndexAfterDamage)
    {
        ...
        {
            aBodyReanim->SetImageOverride("anim_brick", IMAGE_REANIM_ZOMBIE_BRICK2);
        }
        else if (mHelmType == HelmType::HELMTYPE_BRICK && aDamageIndexAfterDamage == 2)
        {
            TOD_ASSERT(aBodyReanim);
            aBodyReanim->SetImageOverride("anim_brick", IMAGE_REANIM_ZOMBIE_BRICK3);
        }
        ...
        else if (mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL && aDamageIndexAfterDamage == 1)
        {
            TOD_ASSERT(aBodyReanim);
            aBodyReanim->SetImageOverride("zombie_football_helmet", IMAGE_REANIM_ZOMBIE_GIGA_FOOTBALL_HELMET2);
        }
        else if (mZombieType == ZombieType::ZOMBIE_GIGA_FOOTBALL && aDamageIndexAfterDamage == 2)
        {
            TOD_ASSERT(aBodyReanim);
            aBodyReanim->SetImageOverride("zombie_football_helmet", IMAGE_REANIM_ZOMBIE_GIGA_FOOTBALL_HELMET3);
        }
...
void Zombie::TakeBodyDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    else if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
    {
        Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        if (aDamageIndexBeforeDamage != aDamageIndexAfterDamage)
        {
            ...
            else if (aDamageIndexAfterDamage == 2)
            {
                ...
                if (mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
                {
                    aBodyReanim->SetImageOverride("anim_head1", IMAGE_TERA2);
                }
                else if (mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
                {
                    aBodyReanim->SetImageOverride("anim_head1", IMAGE_EXA2);
                }
...
void Zombie::TakeDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    {
        theDamage = (theDamage * 25) / 100;
        if (theDamage < 1) theDamage = 1;
    }
    ...
    if (TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_ACID))
    {
        if (aDamageRemaining > 0) TakeBodyDamage(aDamageRemaining, theDamageFlags);
        return;
    }

    if (mSunBanMarked && !IsDeadOrDying() && mHasHead && mBoard->mMainCounter % 100 == 0 && theDamage > 0)
    {
        if (Rand(3) == 0) mBoard->AddCoin(mPosX + mWidth / 2, mPosY + mHeight / 2, CoinType::COIN_SMALLSUN, CoinMotion::COIN_MOTION_FROM_PLANT);
    }
    bool isSunMagnetActive = false;
    Plant* p = nullptr;
    while (mBoard->IteratePlants(p)) {
        if (p->mSeedType == SeedType::SEED_SUN_MAGNET && p->mSunMagnetActive && !p->mDead) {
            isSunMagnetActive = true;
            break;
        }
    }
    if (isSunMagnetActive && !IsDeadOrDying() && mHasHead && theDamage > 0)
    {
        if (Rand(100) < 15) mBoard->AddCoin(mPosX + mWidth / 2, mPosY + mHeight / 2, CoinType::COIN_SMALLSUN, CoinMotion::COIN_MOTION_FROM_PLANT);
    }
    if (mZombieType == ZombieType::ZOMBIE_COCOON)
    {
        if (theDamageFlags != 127 && theDamageFlags != 32U && theDamageFlags != 33U && theDamageFlags != 18U && theDamageFlags != 0U) return;
    }
    ...
    if (aDamageRemaining > 0 && mHelmType != HELMTYPE_NONE)
    {
...
bool Zombie::CanBeChilled()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_NINJA || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
        return false;
...
bool Zombie::CanBeFrozen()
{
    if (!CanBeChilled())
        return false;
    if (mZombieType == ZombieType::ZOMBIE_NINJA) return false;
...
void Zombie::RiseFromGrave(int theCol, int theRow)
{
    ...
    if (mBoard->StageHasPool())
    {
        ...
        else if (mZombieType == ZombieType::ZOMBIE_BRICK && aParticle)
        {
            aBodyReanim->AttachParticleToTrack("anim_brick", aParticle, 37.0f, 20.0f);
        }
        else if (aParticle)
        {
...
bool Zombie::ZombieTypeCanGoInPool(ZombieType theZombieType)
{
    return 
        ...
        theZombieType == ZombieType::ZOMBIE_BRICK ||
        theZombieType == ZombieType::ZOMBIE_IMP ||
        theZombieType == ZombieType::ZOMBIE_DOOR ||
        theZombieType == ZombieType::ZOMBIE_GARGANTUAR ||
        theZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR ||
        theZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR ||
        theZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR ||
...
void Zombie::MowDown()
{
    ...
    else if (mZombieType == ZombieType::ZOMBIE_NEWSPAPER || mZombieType == ZombieType::ZOMBIE_SUNDYEDITION || mZombieType == ZombieType::ZOMBIE_BALLOON)
    {
        DropHead(0U);
    }
    else if (mZombieType == ZombieType::ZOMBIE_POGO)
...
void Zombie::ApplyBurn()
{
    if (mDead || mZombiePhase == ZombiePhase::PHASE_ZOMBIE_BURNED)
        return;
    if (mZombieType == ZombieType::ZOMBIE_BUG || mZombieType == ZombieType::ZOMBIE_FLY || mZombieType == ZombieType::ZOMBIE_COCOON)
    {
        DieNoLoot();
        return;
    }
    bool hasNewspaperShield = (mShieldType == ShieldType::SHIELDTYPE_NEWSPAPER || mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER);
    if (mBodyHealth >= 1800 || mZombieType == ZombieType::ZOMBIE_BOSS || hasNewspaperShield)
    {
        TakeDamage(1800, 18U);
        return;
    }
    ...
    else
    {
        ...
        else if ((mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR) && !mHasObject)
        {
...
void Zombie::AttachShield()
{
    ...
    else if (mShieldType == ShieldType::SHIELDTYPE_NEWSPAPER || mShieldType == ShieldType::SHIELDTYPE_SUNDYNEWSPAPER)
    {
...
void Zombie::UpdateDeath()
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
    {
        if (aBodyReanim->ShouldTriggerTimedEvent(0.89f))
        {
            mBoard->ShakeBoard(0, 3);
            int currentGridX = mBoard->PixelToGridXKeepOnBoard(mX, mY);
            SquishAllInSquare(currentGridX, mRow, ZombieAttackType::ATTACKTYPE_CHEW);
            SquishAllInSquare(currentGridX - 1, mRow, ZombieAttackType::ATTACKTYPE_CHEW);
            SquishAllInSquare(currentGridX - 2, mRow, ZombieAttackType::ATTACKTYPE_CHEW);
        }
...
void Zombie::DrawShadow(Graphics* g)
{
    ...
    else if (mZombieType == ZombieType::ZOMBIE_BUG)
    {
        ...
        if (IsWalkingBackwards())
        {
            aShadowOffsetX += 12.0f * mScaleZombie;
            aShadowOffsetY += 3000.0f;
        }
        else
        {
            aShadowOffsetX += 12.0f * mScaleZombie;
        ...
        aShadowOffsetY = (120.0f * mScaleZombie) + 1.0f - 92.0f;
    }
    else if (mZombieType == ZombieType::ZOMBIE_SUNDYEDITION)
    {
        if (IsWalkingBackwards())
    ...
    else if (mZombieType == ZombieType::ZOMBIE_COCOON)
    {
        aScale *= 0.0001f;
        aShadowOffsetY += 7.0f;
        if (IsWalkingBackwards())
    ...
    else if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR)
    {
        aScale *= 1.5f;
...
void Zombie::WalkIntoHouse()
{
        ...
        mBoard->mBackground == BackgroundType::BACKGROUND_3_POOL || mBoard->mBackground == BackgroundType::BACKGROUND_4_FOG)
    {
        ...
        if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR)
        {
            mPosY += 30.0f;
        }
    ...
    else if (mBoard->mBackground == BackgroundType::BACKGROUND_5_ROOF || mBoard->mBackground == BackgroundType::BACKGROUND_6_BOSS)
    {
        ...
        if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR)
        {
            mPosY += 5.0f;
        }
        else if (mZombieType == ZombieType::ZOMBIE_FOOTBALL)
...
void Zombie::BossHeadSpit()
{
    ...
    int attackType = RandRangeInt(0, 3); 
    mIsFireBall = (attackType == 0);
    mIsNuclearBall = (attackType == 2);
    mIsVacuumBall = (attackType == 3);
...
void Zombie::UpdateBoss()
{
    ...
    else if (mZombiePhase == ZombiePhase::PHASE_BOSS_HEAD_SPIT)
    {
        if (mIsNuclearBall)
        {
            ...
            mChilledCounter = 0;
            mButteredCounter = 0;
            if (mNuclearDelayCounter > 0)
            {
                mNuclearDelayCounter--;
                if (mNuclearDelayCounter == 0)
                {
                    mBoard->mNuclearRainActive = true;
                    mBoard->mNuclearRainCounter = 0;
                }
            }
            if (mBoard->mNuclearRainActive && mBoard->mNuclearRainCounter < 300)
            {
                if (aBodyReanim->mAnimTime >= 0.42f && aBodyReanim->mAnimRate != 0.0f)
                {
                    aBodyReanim->mAnimRate = 0.0f;
                }
            }
            else if (aBodyReanim->mAnimRate == 0.0f)
            {
        ...
        {
            mIceTrapCounter = 0; mChilledCounter = 0; mButteredCounter = 0;
            if (aBodyReanim->mAnimTime >= 0.42f && aBodyReanim->mAnimRate != 0.0f)
            {
                aBodyReanim->mAnimRate = 0.0f;
            }
            if (aBodyReanim->mAnimRate == 0.0f)
            {
                bool plantsLeft = false;
                Plant* aPlant = nullptr;
                while (mBoard->IteratePlants(aPlant))
                {
                    if (aPlant->mRow == mFireballRow && !aPlant->mDead && !aPlant->NotOnGround())
                    {
                        plantsLeft = true;
                        aPlant->mX += 2;
                        aPlant->mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 1);
                        if (mBoard->StageHasRoof()) {
                            aPlant->mY = mBoard->GetPosYBasedOnRow(aPlant->mX, aPlant->mRow);
                        }
                        if (aPlant->mX >= 825.0f)
                        {
                            TodParticleSystem* p = mApp->AddTodParticle(aPlant->mX, aPlant->mY, aPlant->mRenderOrder + 1, ParticleEffect::PARTICLE_FIREBALL_DEATH);
                            if (p)
                            {
                                p->OverrideColor(nullptr, Color(160, 32, 240, 255));
                                p->OverrideScale(nullptr, 0.75f);
                            }
...
void Zombie::PreloadZombieResources(ZombieType theZombieType)
{
    ...
    else if (theZombieType == ZombieType::ZOMBIE_BUG)
    {
        ReanimatorEnsureDefinitionLoaded(GetZombieDefinition(ZombieType::ZOMBIE_COCOON).mReanimationType, true);
    }
    else if (theZombieType == ZombieType::ZOMBIE_COCOON)
    {
        ReanimatorEnsureDefinitionLoaded(GetZombieDefinition(ZombieType::ZOMBIE_BUG).mReanimationType, true);
    }
    else if (theZombieType == ZombieType::ZOMBIE_FLY)
    {
        ReanimatorEnsureDefinitionLoaded(GetZombieDefinition(ZombieType::ZOMBIE_BUG).mReanimationType, true);
    }
    ...
    else if (theZombieType == ZombieType::ZOMBIE_GARGANTUAR || theZombieType == ZombieType::ZOMBIE_BLEUEYE_GARGANTUAR || theZombieType == ZombieType::ZOMBIE_EXA_GARGANTUAR || theZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR)
    {
...
void Zombie::EnableDance(bool theEnableDance)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_NORMAL || mZombieType == ZombieType::ZOMBIE_TRAFFIC_CONE || mZombieType == ZombieType::ZOMBIE_PAIL || mZombieType == ZombieType::ZOMBIE_BRICK)
    {
        StartWalkAnim(0);
    }
}
...
void Zombie::UpdateAcid()
{
    if (mAcidCounter > 0 && !IsDeadOrDying())
    {
        mAcidCounter--;
        mAcidDamageCounter++;
        if (mAcidDamageCounter >= 52)
        {
            mAcidDamageCounter = 0;
            unsigned int aAcidFlags = 3U; 
            TakeBodyDamage(15, aAcidFlags);
        }
    }
}
bool Zombie::IsTooStrongForBees()
{
    if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR ||
        mZombieType == ZombieType::ZOMBIE_BOSS ||
        mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR ||
...
void Zombie::ApplyNovaBurn()
{
    if (!mIsNovaBurning)
    {
        mIsNovaBurning = true;
        mNovaBurnTimer = 50;
        AddAttachedParticle(40, 60, ParticleEffect::PARTICLE_FIREBALL_TRAIL);
    }
}
void Zombie::UpdateZombieNinja()
{
    if (mDead || mBoard == nullptr || mZombiePhase == ZombiePhase::PHASE_ZOMBIE_DYING)
        return;

    if (mNinjaIsTeleporting)
    {
        mNinjaTeleportDelayTimer++;
        mVisible = false;
        mVelX = 0;

        if (mNinjaTeleportDelayTimer == 75)
        {
            mRow = mNinjaTargetRow;
            mX = mNinjaTargetX;
            mPosY = GetPosYBasedOnRow(mRow); // إصلاح الارتفاع ليكون مطابقاً للـ Row تماماً
            mY = (int)mPosY;
            mPosX = mX;
            mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, mRow, 0);
            Reanimation* aAnim = mApp->ReanimationGet(mBodyReanimID);
            if (aAnim)
            {
                aAnim->SetPosition(mPosX, mPosY);
                aAnim->Update();
            }
        }

        if (mNinjaTeleportDelayTimer >= 100)
        {
            mNinjaIsTeleporting = false;
            mNinjaTeleportDelayTimer = 0;
            mVisible = true;
            mNinjaTeleportTimer = 0;

            TodParticleSystem* aPart = mApp->AddTodParticle(mPosX + 20.0f, mPosY, mRenderOrder + 1, ParticleEffect::PARTICLE_FIREBALL_DEATH);
            if (aPart) aPart->OverrideColor(nullptr, Color(0, 0, 0, 255));
        }
        return;
    }

    bool aBlockedByWall = false;
    int aTargetGridX = (int)((mX + 40) / 80);
    Plant* p = nullptr;
    while (mBoard->mPlants.IterateNext(p))
    {
        int aPlantGridX = (int)(p->mX / 80);
        if (!p->mDead && p->mRow == mRow && aPlantGridX == aTargetGridX)
        {
            if (p->mSeedType == SeedType::SEED_WALLNUT ||
                p->mSeedType == SeedType::SEED_TALLNUT ||
                p->mSeedType == SeedType::SEED_PUMPKINSHELL)
            {
                aBlockedByWall = true;
                break;
            }
    ...
    mNinjaTeleportTimer++;
    int aThreshold = aBlockedByWall ? 50 : 400;

    if (mNinjaTeleportTimer >= aThreshold)
    {
        int aActionChance = Rand(100);
        if (aBlockedByWall) aActionChance = 80;

        if (aActionChance < 60 && !aBlockedByWall)
        {
            Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
            if (aBodyReanim)
            {
                int aTrackIndex = aBodyReanim->FindTrackIndex("anim_body");
                if (aTrackIndex == -1) aTrackIndex = aBodyReanim->FindTrackIndex("anim_torso");

                ReanimatorTransform aTransform;
                aBodyReanim->GetCurrentTransform(aTrackIndex, &aTransform);

                float aOriginX = mPosX + aTransform.mTransX;
                float aOriginY = mPosY + aTransform.mTransY - mAltitude + 40.0f;

                Projectile* p1 = mBoard->AddProjectile((int)aOriginX, (int)aOriginY, mRenderOrder + 1, mRow, ProjectileType::PROJECTILE_NINJA);
                if (p1)
                {
                    p1->mVelX = -5.0f;
                    p1->mVelY = 0.0f;
                    p1->mRotation = 0.0f;
                    p1->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
                }

                Projectile* p2 = mBoard->AddProjectile((int)aOriginX, (int)aOriginY, mRenderOrder + 1, mRow, ProjectileType::PROJECTILE_NINJA);
                if (p2)
                {
                    p2->mVelX = -3.5f;
                    p2->mVelY = 0.0f;
                    p2->mRotation = 0.0f;
                    p2->mMotionType = ProjectileMotion::MOTION_BACKWARDS;
                }
        ...
        {
            Plant* aTempPlants[1024];
            int aPlantCount = 0;
            Plant* p2 = nullptr;
            Plant* aBeehiveTarget = nullptr;

            while (mBoard->IteratePlants(p2))
            {
                if (p2 && !p2->mDead && p2->mRow == mRow)
                {
                    if (p2->mSeedType == SeedType::SEED_BEEHIVE)
                    {
                        aBeehiveTarget = p2; // أعلى أولوية
                        break;
                    }
                    // يتجاهل أي نبتة دفاعية تماماً
                    if (p2->mSeedType != SeedType::SEED_WALLNUT &&
                        p2->mSeedType != SeedType::SEED_TALLNUT &&
                        p2->mSeedType != SeedType::SEED_GIANT_WALLNUT &&
                        p2->mSeedType != SeedType::SEED_EXPLODE_O_NUT &&
                        p2->mSeedType != SeedType::SEED_PUMPKINSHELL)
                    {
                        if (aPlantCount < 1024) aTempPlants[aPlantCount++] = p2;
                    }
                }
            }
            ...
            if (aBeehiveTarget)
            {
                ...
                mNinjaTargetX = aBeehiveTarget->mX - 20.0f;
                mNinjaTargetRow = aBeehiveTarget->mRow;
                mNinjaIsTeleporting = true;
            }
            else if (aPlantCount > 0)
            {
                int aIdx = Rand(aPlantCount);
                Plant* aTargetPlant = aTempPlants[aIdx];

                TodParticleSystem* aPart = mApp->AddTodParticle(mPosX + 20.0f, mPosY, mRenderOrder + 1, ParticleEffect::PARTICLE_FIREBALL_DEATH);
                if (aPart) aPart->OverrideColor(nullptr, Color(0, 0, 0, 255));

                mNinjaTargetX = aTargetPlant->mX - 20.0f;
                mNinjaTargetRow = aTargetPlant->mRow;
                mNinjaIsTeleporting = true;
            }
        }
    }
