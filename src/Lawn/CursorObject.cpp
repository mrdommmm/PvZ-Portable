...
void CursorObject::Draw(Graphics* g)
{
    ...
    case CursorType::CURSOR_TYPE_SHOVEL:
        ...
        break;
    case CursorType::CURSOR_TYPE_BOOSTER:
        g->DrawImage(Sexy::IMAGE_BOOSTER, 10, -30);
        break;
    ...
    case CursorType::CURSOR_TYPE_PLANT_FROM_GLOVE:
    {
        ...
        {
            if (mGlovePlantID != PlantID::PLANTID_NULL)
            {
                Plant* aPlant = mBoard->mPlants.DataArrayTryToGet((unsigned int)mGlovePlantID);
                if (aPlant)
                {
        ...
        else
        {
            if (mGlovePlantID != PlantID::PLANTID_NULL)
            {
                ...
                if (aPlant)
                {
                    // ÇáÍá: ÇÓÊÏÚÇÁ ÏÇáÉ ÇáÑÓã ÇáÕÍíÍÉ ááÚÈ ÇáÚÇÏí
                    Plant::DrawSeedType(g, aPlant->mSeedType, aPlant->mImitaterType, DrawVariation::VARIATION_NORMAL, -22.0f, -15.0f);
                }
            }
        }
        break;
...
void CursorPreview::Draw(Graphics* g)
{
    ...
    {
        // ÅÖÇÝÉ ÔÑØ ÇáÒä ÌÇÑÏä áãäÚ ÊÍæá ÇáÙá ÇáÔÝÇÝ áÃÕíÕ Ýí ÇááÚÈ ÇáÚÇÏí
        if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN)
        {
