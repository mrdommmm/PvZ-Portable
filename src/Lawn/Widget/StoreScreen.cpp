...
static StoreItem gStoreItemSpots[NUM_STORE_PAGES][MAX_PAGE_SPOTS] =
{
    ...
    { STORE_ITEM_PACKET_UPGRADE,    STORE_ITEM_POOL_CLEANER,        STORE_ITEM_RAKE,                STORE_ITEM_ROOF_CLEANER,
      ...
      { STORE_ITEM_PLANT_SPIKEROCK,   STORE_ITEM_PLANT_GOLD_MAGNET,   STORE_ITEM_PLANT_WINTERMELON,   STORE_ITEM_PLANT_COBCANNON,
        ...
        // Page 3
        { STORE_ITEM_SHOVEL_UPGRADE,    STORE_ITEM_DIAMOND_SHOVEL,      STORE_ITEM_COSMIC_SHOVEL,       STORE_ITEM_BOARD_BOOSTER,
          STORE_ITEM_BOARD_FERTILIZER,  STORE_ITEM_BOARD_GLOVE,         STORE_ITEM_INVALID,             STORE_ITEM_INVALID },
          ...
          { STORE_ITEM_POTTED_MARIGOLD_1, STORE_ITEM_POTTED_MARIGOLD_2,   STORE_ITEM_POTTED_MARIGOLD_3,   STORE_ITEM_GOLD_WATERINGCAN,
            ...
            { STORE_ITEM_MUSHROOM_GARDEN,   STORE_ITEM_AQUARIUM_GARDEN,     STORE_ITEM_WHEEL_BARROW,        STORE_ITEM_STINKY_THE_SNAIL,
              ...
              // Page 6 (ÇáÕÝÍÉ ÇáÌÏíÏÉ ááäÈÇÊÇÊ)
              { STORE_ITEM_PLANT_SUNPEA,      STORE_ITEM_PLANT_GOOPEA,        STORE_ITEM_PLANT_ROCKPEA,       STORE_ITEM_PLANT_CPEA,
                STORE_ITEM_PLANT_DROPPEA,     STORE_ITEM_PLANT_PRIMALPOTATOMINE, STORE_ITEM_PLANT_PRIMALSUNFLOWER, STORE_ITEM_INVALID }
};
...
void StoreScreen::DrawItemIcon(Graphics* g, int theItemPosition, StoreItem theItemType, bool theIsForHighlight)
{
    ...
    else if (theItemType == STORE_ITEM_BOARD_FERTILIZER)
    {
        g->DrawImage(Sexy::IMAGE_FERTILIZER, aPosX - 11, aPosY - 2);
    }
    else if (theItemType == STORE_ITEM_BOARD_GLOVE)
    {
        g->DrawImage(Sexy::IMAGE_ZEN_GARDENGLOVE, aPosX - 12, aPosY + 3);
    }
    else if (theItemType == STORE_ITEM_BOARD_BOOSTER)
    {
        g->DrawImage(Sexy::IMAGE_BOOSTER, aPosX - 12, aPosY + 3);
    }
    else if (IsPottedPlant(theItemType))
    {
        mApp->mZenGarden->DrawPottedPlantIcon(g, aPosX, aPosY, &mPottedPlantSpecs);
    }
    else if (theItemType == STORE_ITEM_SHOVEL_UPGRADE)
    {
        g->DrawImage(Sexy::IMAGE_SHOVEL_GOLD, aPosX - 5, aPosY + 10);
    }
    else if (theItemType == STORE_ITEM_DIAMOND_SHOVEL)
    {
        g->DrawImage(Sexy::IMAGE_SHOVEL_DIAMOND, aPosX - 5, aPosY + 10);
    }
    else if (theItemType == STORE_ITEM_COSMIC_SHOVEL)
    {
        g->DrawImage(Sexy::IMAGE_SHOVEL_COSMIC, aPosX - 5, aPosY + 10);
        }
    else
    {
        DrawSeedPacket(g, aPosX, aPosY, (SeedType)(theItemType + 55), SEED_NONE, 0, 255, false, false);
    }
...
void StoreScreen::DrawOverlay(Graphics* g)
{
    ...
    // ÅÖÇÝÉ ÊÃËíÑ ÇáãØÑ æÇáÝáÊÑ
    g->SetColor(Color(0, 0, 0, 90));
    g->FillRect(0, 0, BOARD_WIDTH, BOARD_HEIGHT);

    g->SetColorizeImages(true);
    g->SetColor(Color(150, 180, 200, 200));

    int aTime = mUpdateCnt % 100;
    int aTimeOffsetXEst = TodAnimateCurve(0, 100, aTime, 0, -100, TodCurves::CURVE_LINEAR);
    int aTimeOffsetYEst = TodAnimateCurve(0, 20, aTime % 20, -100, 0, TodCurves::CURVE_LINEAR);

    for (int aHorCnt = 0; aHorCnt < 15; aHorCnt++)
    {
        for (int aVerCnt = 0; aVerCnt < 9; aVerCnt++)
        {
            g->DrawImage(Sexy::IMAGE_RAIN, aHorCnt * 100 + aTimeOffsetXEst, aVerCnt * 100 + aTimeOffsetYEst);
        }
    }

    aTime = mUpdateCnt;
    float aTimeOffsetXCls = TodAnimateCurve(0, 161, aTime % 161, 0, -100, TodCurves::CURVE_LINEAR);
    float aTimeOffsetYCls = TodAnimateCurve(0, 33, aTime % 33, -100, 0, TodCurves::CURVE_LINEAR);

    for (int aHorCnt = 0; aHorCnt < 8; aHorCnt++)
    {
        for (int aVerCnt = 0; aVerCnt < 5; aVerCnt++)
        {
            TodDrawImageScaledF(g, Sexy::IMAGE_RAIN, (aHorCnt * 200 + aTimeOffsetXCls), (aVerCnt * 200 + aTimeOffsetYCls), 1.5f, 1.5f);
        }
    }
    g->SetColorizeImages(false);
}
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (aItemType != STORE_ITEM_INVALID && !IsItemUnavailable(aItemType))
        {
            ...
            if (Rect(aItemX, aItemY, 50, 87).Contains(aMouseX, aMouseY))
            {
                ...
                case STORE_ITEM_SHOVEL_UPGRADE:         aMessageIndex = 2035;                           break;
                case STORE_ITEM_BOARD_FERTILIZER:       aMessageIndex = 2036;                           break;
                case STORE_ITEM_BOARD_GLOVE:            aMessageIndex = 2037;                           break;
                case STORE_ITEM_BOARD_BOOSTER:          aMessageIndex = 2038;                           break;
                case STORE_ITEM_DIAMOND_SHOVEL:         aMessageIndex = 2039;                           break; 
                case STORE_ITEM_COSMIC_SHOVEL:          aMessageIndex = 2040;                           break;
                case STORE_ITEM_PLANT_SUNPEA:           aMessageIndex = 2041;                           break;
                case STORE_ITEM_PLANT_GOOPEA:           aMessageIndex = 2042;                           break;
                case STORE_ITEM_PLANT_ROCKPEA:          aMessageIndex = 2043;                           break;
                case STORE_ITEM_PLANT_CPEA:             aMessageIndex = 2044;                           break;
                case STORE_ITEM_PLANT_DROPPEA:          aMessageIndex = 2045;                           break;
                case STORE_ITEM_PLANT_PRIMALPOTATOMINE: aMessageIndex = 2046;                           break;
                case STORE_ITEM_PLANT_PRIMALSUNFLOWER:  aMessageIndex = 2047;                           break;
...
bool StoreScreen::IsPageShown(StorePages thePage)
{
    ...
    if (thePage == STORE_PAGE_NEW_UPGRADES) return mApp->mPlayerInfo->mLevel >= 10;
...
int StoreScreen::GetItemCost(StoreItem theStoreItem)
{
    ...
    case STORE_ITEM_PLANT_GLOOMSHROOM:                  return 1000;
    case STORE_ITEM_PLANT_CATTAIL:                      return 1500;
    case STORE_ITEM_PLANT_WINTERMELON:                  return 2500;
    case STORE_ITEM_PLANT_GOLD_MAGNET:                  return 100;
    case STORE_ITEM_PLANT_SPIKEROCK:                    return 1500;
    case STORE_ITEM_PLANT_COBCANNON:                    return 5000;
    case STORE_ITEM_PLANT_IMITATER:                     return 7500;
    ...
    case STORE_ITEM_PACKET_UPGRADE:
    {
        ...
        return aPurchase == 0 ? 75 : aPurchase == 1 ? 500 : aPurchase == 2 ? 2000 :
            aPurchase == 3 ? 8000 : aPurchase == 4 ? 16000 : 32000;
    }
    ...
    case STORE_ITEM_SHOVEL_UPGRADE:                     return 350;
    case STORE_ITEM_BOARD_FERTILIZER:                   return 1000;
    case STORE_ITEM_BOARD_GLOVE:                        return 500;
    case STORE_ITEM_BOARD_BOOSTER:                      return 500;
    case STORE_ITEM_DIAMOND_SHOVEL:                     return 2500; 
    case STORE_ITEM_COSMIC_SHOVEL:                      return 20000; 
    case STORE_ITEM_PLANT_SUNPEA:                       return 200;
    case STORE_ITEM_PLANT_GOOPEA:                       return 250;
    case STORE_ITEM_PLANT_ROCKPEA:                      return 300;
    case STORE_ITEM_PLANT_CPEA:                         return 450;
    case STORE_ITEM_PLANT_DROPPEA:                      return 300;
    case STORE_ITEM_PLANT_PRIMALPOTATOMINE:             return 500;
    case STORE_ITEM_PLANT_PRIMALSUNFLOWER:              return 750;
...
void StoreScreen::MouseDown(int x, int y, int theClickCount)
{
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (Rect(aItemX, aItemY, 50, 87).Contains(x, y))
        {
            ...
            else if (IsItemSoldOut(aItemType)) // ÇáÃÏÇÉ ãÔÊÑÇÉ ÈÇáÝÚá
            {
                // ÍÓÇÈ äÕÝ ÇáÓÚÑ
                int aRefundAmount = GetItemCost(aItemType) / 2;
                SexyString aPrompt = StrFormat(_S("Do you want to sell this item back for %d coins?"), aRefundAmount);

                LawnDialog* aSellDialog = (LawnDialog*)mApp->DoDialog(
                    DIALOG_STORE_PURCHASE, true, _S("Sell Item"), aPrompt, _S(""), Dialog::BUTTONS_YES_NO);

                aSellDialog->mLawnYesButton->mLabel = _S("[DIALOG_BUTTON_YES]");
                aSellDialog->mLawnNoButton->mLabel = _S("[DIALOG_BUTTON_NO]");

                mWaitForDialog = true;
                int aResult = aSellDialog->WaitForResult(true);
                mWaitForDialog = false;

                if (aResult == Dialog::ID_YES)
                {
                    // ÓÍÈ ÇáÃÏÇÉ æÅÑÌÇÚ äÕÝ ÇáÓÚÑ
                    mApp->mPlayerInfo->mPurchases[aItemType] = 0;
                    mApp->mPlayerInfo->AddCoins(aRefundAmount);
                    mApp->PlaySample(Sexy::SOUND_COIN);
                    mApp->WriteCurrentUserConfig();

                    // ÊÍÏíË ÇááæÍÉ ÅÐÇ ßäÇ ÏÇÎá ãÑÍáÉ
                    if (mApp->mBoard) {
                        mApp->mBoard->mSpecialShovelCooldown = 0; // ÊÕÝíÑ ÇáÚÏÇÏÇÊ ÇÍÊíÇØíÇð
                    }
                }
            }
            break;
