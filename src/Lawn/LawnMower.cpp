...
void LawnMower::LawnMowerInitialize(int theRow)
{
    ...
    for (int i = 0; i < 128; i++)
    {
        mMowedZombies[i] = ZombieID::ZOMBIEID_NULL;
    }
...
void LawnMower::Update()
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
            ...
            aZombie->EffectedByDamage(127U))
        {
            ...
            if (aOverlap > (aZombie->mZombieType == ZombieType::ZOMBIE_BALLOON ? 20 : 0))
            {
                ...
                {
                    bool isAlreadyMowed = false;
                    ZombieID aZombieID = mBoard->ZombieGetID(aZombie);

                    for (int i = 0; i < mNumMowedZombies; i++)
                    {
                        if (mMowedZombies[i] == aZombieID)
                        {
                            isAlreadyMowed = true;
                            break;
                        }
                    ...
                    if (!isAlreadyMowed)
                    {
                        ...
                        if (mNumMowedZombies < 128)
                        {
                            mMowedZombies[mNumMowedZombies] = aZombieID;
                            mNumMowedZombies++;
                        }
                    }
                }
    ...
    if (mMowerState == LawnMowerState::MOWER_TRIGGERED)
    {
        ...
        {
            // ÇáÊÍÞÞ ãä Ãä ÇáäÈÊÉ Ýí äÝÓ ÇáÕÝ æáã ÊãÊ ÈÚÏ
            if (aPlant->mRow == mRow && !aPlant->mDead && !aPlant->mSquished)
            {
                // ÈäÇÁ ãÓÊØíá ÇáÂáÉ (ÚÏá ÇáãÊÛíÑÇÊ ÍÓÈ ßáÇÓ LawnMower áÏíß)
                Rect aMowerRect = Rect(mPosX, mPosY, 80, 80);
                Rect aPlantRect = aPlant->GetPlantRect();

                // ÅÐÇ ÍÕá ÊÏÇÎá Èíä ÇáÂáÉ æÇáäÈÊÉ
                if (GetRectOverlap(aMowerRect, aPlantRect) > 20)
                {
                    if (aPlant->mSeedType == SeedType::SEED_SPIKEWEED)
                    {
                        // ãæÊ ÓÈÇíß æíÏ æÇáÂáÉ æÇäÝÌÇÑ ÌÇß
                        aPlant->Die();
                        mApp->AddTodParticle(mPosX + 40, mPosY + 40, mRenderOrder + 1, ParticleEffect::PARTICLE_JACKEXPLODE);
                        mBoard->ShakeBoard(3, -4);
                        mApp->PlayFoley(FoleyType::FOLEY_EXPLOSION);

                        this->Die(); // ÇÓÊÈÏá ÈÜ mMowerState = LawnMowerState::MOWER_SQUISHED ÅÐÇ ßÇäÊ åßÐÇ Ýí ßæÏß
                        break;
                    }
                    ...
                    else if (mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL)
                    {
                        ...
                        Reanimation* aSplash = mApp->AddReanimation(aPlant->mX - 11, aPlant->mY + 20, aPlant->mRenderOrder + 1, ReanimationType::REANIM_SPLASH);
                        if (aSplash) aSplash->OverrideScale(0.8f, 0.8f);

                        mApp->AddTodParticle(aPlant->mX + 40, aPlant->mY + 40, aPlant->mRenderOrder + 1, ParticleEffect::PARTICLE_PLANTING_POOL);
