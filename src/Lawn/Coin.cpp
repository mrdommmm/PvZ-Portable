...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    else if (mType == CoinType::COIN_PLANT_FOOD)
    {
        mWidth = IMAGE_BOOSTER->GetCelWidth();
        mHeight = IMAGE_BOOSTER->GetCelHeight();
        mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 0);
    }
    ...
    if (IsPlantFood())
    {
        float aAngle = RandRangeFloat(0, 2 * PI);
        float aSpeed = 0.5f;
        mVelX = cos(aAngle) * aSpeed;
        mVelY = sin(aAngle) * aSpeed;
    }
    ...
    else if (mType == CoinType::COIN_BIGSUN)
    {
        aScale = 2.0f;
    }
...
bool Coin::IsSun()
{
    return mType == CoinType::COIN_SUN || mType == CoinType::COIN_SMALLSUN || mType == CoinType::COIN_BIGSUN;
}
...
void Coin::ScoreCoin()
{
    ...
    if (mType == CoinType::COIN_PLANT_FOOD)
    {
        if (mBoard->mPlantFoodCount < 3)
        {
            mBoard->mPlantFoodCount++;
        }
        else
        {
            mBoard->mPlantFoodCount = 3;
        }
        mApp->PlaySample(Sexy::SOUND_MINDCONTROLLED);
    }
...
void Coin::UpdateFall()
{
    if (IsPlantFood())
    {
        ...
        if (mPosX < 0) { mPosX = 0; mVelX = -mVelX; }
        if (mPosX > BOARD_WIDTH - mWidth) { mPosX = BOARD_WIDTH - mWidth; mVelX = -mVelX; }
        if (mPosY < 0) { mPosY = 0; mVelY = -mVelY; }
        if (mPosY > BOARD_HEIGHT - mHeight) { mPosY = BOARD_HEIGHT - mHeight; mVelY = -mVelY; }
...
void Coin::UpdateCollected()
{
    ...
    {
        Rect aShovelRect = mBoard->GetShovelButtonRect();
...
void Coin::Draw(Graphics* g)
{
    ...
    if (IsPlantFood())
    {
        ...
        Color aGlowColor = Color(255, 255, 255, GetColor().mAlpha); 
...
{
    return mType == CoinType::COIN_SMALLSUN ? 0.5f : mType == CoinType::COIN_BIGSUN ? 2.0f : 1.0f;
}

int Coin::GetSunValue()
{
    return mType == CoinType::COIN_SUN ? 25 : mType == CoinType::COIN_SMALLSUN ? 15 : mType == CoinType::COIN_BIGSUN ? 50 : 0;
}

int Coin::GetCoinValue(CoinType theCoinType)
{
    return theCoinType == CoinType::COIN_SILVER ? 2 : theCoinType == CoinType::COIN_GOLD ? 10 : theCoinType == CoinType::COIN_DIAMOND ? 250 : 0;
}
...
void Coin::PlayLaunchSound()
{
    if (mType == CoinType::COIN_PLANT_FOOD)
    {
        mApp->PlayFoley(FoleyType::FOLEY_PLANTFOOD);
        return;
    }
    if (mType == CoinType::COIN_DIAMOND || 
        ...
        if (IsPlantFood())
        {
            if (mBoard->mPlantFoodCount >= 3)
            {
                ...
                mBoard->DisplayAdvice(
                    _S("Your Plant Food is already full!"),
...
int Coin::GetDisappearTime()
{
    ...
    if (mType == CoinType::COIN_DIAMOND || mType == CoinType::COIN_PRESENT_PLANT || mType == CoinType::COIN_CHOCOLATE || mType == CoinType::COIN_PLANT_FOOD || mHasBouncyArrow)
    {
        aTime = 1500;
    }
